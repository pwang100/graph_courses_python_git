...
```{python}
jobs.to_csv("../data/glassdoor_jobs_minyrsreq.csv", index=False)
```
...
px.scatter(x = jobs["years_required"],
            y = jobs["midpoint_salary_estimate"]
...
It seems like most jobs posted on Glassdoor require under 5 years work experience. It could be that senior jobs requiring more than 5 years experience are not usually posted on Glassdoor and are recruited by headhunters.
...
def lang_req(description):
    prompt = f"In the job description, extract if the job requires only the programming languages R or Python. Return only a string and one of these four categories: 'R', 'Python', 'Both' or 'Neither'. Here is the job description: {description}."
...
There is a pattern that jobs requiring only python or both python and R have the highest median salary. Jobs requiring only R or neither have the lowest median salary.
