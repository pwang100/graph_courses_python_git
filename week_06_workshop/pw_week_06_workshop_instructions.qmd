...
```{python}
pa = pd.read_csv("/Users/priscillawang/Desktop/graph_courses_python/data/physical_activity.csv")
pa
```
...
```{python}
sec_to_min_vec = np.vectorize(sec_to_min)
```

```{python}
pa["sedentary_ap_m_wk"] = sec_to_min_vec(pa["sedentary_ap_s_day"])
pa["light_ap_m_wk"] = sec_to_min_vec(pa["light_ap_s_day"])
pa["mvpa_m_wk"] = sec_to_min_vec(pa["mvpa_s_day"])
...
```{python}
pa["total_pa_m_wk"] = pa["sedentary_ap_m_wk"] + pa["light_ap_m_wk"] + pa["mvpa_m_wk"]
pa
```
...
```{python}
def activity_status(total_pa_m_wk):
   if total_pa_m_wk >=0 and total_pa_m_wk < 500:
      return "low"
...
```{python}
activity_status_vec = np.vectorize(activity_status)
```

```{python}
pa["total_pa_cat"] = activity_status_vec(pa["total_pa_m_wk"])
pa
```
...
Males and females have approximatly the same total median physical activity per week, while there are some fitness enthusiast outliers of men who have significantly higher physical activity than female outliers. 
